 ...
 * This is generated by the getLayout utility function.
 *
...
const GraphBase = ({
    ...
    useEffect(() => {
        Object.keys(axes).forEach(position => {
            const axisData = axes[position];
            const axis = axesFunctions[position]
            ...
            if (axisRefs[position].current) {
                select(axisRefs[position].current).call(axis);
            }
        });
    }, []);
